...
const schema = z.object({
  ...
  schedule_id: z.string().min(1, "Pilih jam mengajar"),
...
export type SavedJournal = {
  ...
  schedule?: string; schedule_id?: string;
...
function Wizard() {
  ...
  function validStep(s: number): boolean {
    ...
    if (s === 1 && (!scheduleId || material.trim().length < 3)) { toast.error("Pilih jam mengajar dan isi materi."); return false; }
  ...
  async function save() {
    const parsed = schema.safeParse({
      class_id: classId, subject_id: subjectId, schedule_id: scheduleId,
    ...
    try {
      ...
      async function persistJournal(): Promise<{ jid: string; photo_url: string; signature_url: string }> {
        ...
        await setDocTo("journals", jid, {
          teacher_id: user?.id || "", class_id: classId, subject_id: subjectId, schedule_id: scheduleId,
                  ...
                  {teacherStatus === "sakit" && (
                    <div className="space-y-3 rounded-2xl bg-amber-50/70 p-3.5">
                      <div>
                        ...
                        <label className="inline-flex cursor-pointer items-center gap-2 rounded-xl bg-ink px-4 py-2.5 text-sm font-semibold text-white">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="mt-2 flex items-center gap-2">
                      <button onClick={lockSig} className="inline-flex flex-1 items-center justify-center gap-1.5 rounded-xl bg-emerald-600 px-4 py-2.5 text-sm font-bold text-white transition hover:bg-emerald-700">
                      ...
                      <li key={s.id} className="rounded-2xl border border-slate-100 bg-slate-50/50 p-3">
                        ...
                        <div className="mt-2 grid grid-cols-4 gap-1.5">
                          {(["Hadir", "Sakit", "Izin", "Alpha"] as Status[]).map((st) => (
                            <label key={st} className={cn("cursor-pointer rounded-lg border px-1 py-1.5 text-center text-xs font-bold transition",
